...
#Step 1
# 生成服务器keystore(密钥和证书)
keytool -genkey -alias localhost -keypass 123456 -keyalg RSA -keysize 1024 -validity 365 -keystore server.keystore.jks -storepass 123456 -dname "CN=mrray.cn, OU=mrray, O= XY Inc., L=CD, ST=SC, C=CN"
# 生成客户端keystore(密钥和证书)
keytool -genkey -alias localhost -keypass 123456 -keyalg RSA -keysize 1024 -validity 365 -keystore client.keystore.jks -storepass 123456 -dname "CN=mrray.cn, OU=mrray, O= XY Inc., L=CD, ST=SC, C=CN"

#Step 2
# 创建CA证书
openssl req -new -x509 -keyout ca-key -out ca-cert -days 365 -passin pass:123456 -passout pass:123456 -subj "/C=CN/ST=SC/L=CD/O=XY Inc./OU=mrray/CN=mrray.cn"
